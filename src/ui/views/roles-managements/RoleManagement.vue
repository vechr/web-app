        ...
        <div v-else class="center-screen">
          ...
          >
            <template #description>
              <span> There's no roles found! </span>
            </template>
          </a-empty>
        </div>
        <!-- Pagination -->
...
export default defineComponent({
  ...
  setup() {
    ...
    const permission = usePermissionManagementStore();
    ...
    const onChangePagination = (pageNumber: number, pageSize: number) => {
      ...
      store.getRolePagination(params);
    };
    ...
    onBeforeMount(() => {
      permission.getOptionPermission();
      store.getRolePagination(params);
    });

    const onDelete = (record: Partial<Role>) => {
      if (record.id) store.deleteRoleById(record.id);
    };

    const onEdit = (record: any) => {
      common.setIsDrawerShow(true);
      store.getRoleById(record.id);
    };
