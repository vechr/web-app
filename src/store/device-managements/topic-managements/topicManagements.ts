...
export const useTopicManagementStore = defineStore('topicManagement', {
  ...
  actions: {
    ...
    async createTopic<T>(
      ...
      const common = useCommonStore();
      await axios
        .post(`/device/${deviceId}/topic`, JSON.stringify(value, null, 2))
        .then((res) => {
    ...
    async updateTopicById<T>(
      ...
      const common = useCommonStore();
      await axios
        .patch(`/device/${deviceId}/topic/${id}`, value)
        .then((res) => {
    ...
    async deleteTopicById<T>(deviceId: T, id: string) {
      await axios
        .delete(`/device/${deviceId}/topic/${id}`)
        .then((res) => {
          if (res.status === 200) {
            this.message = res.data.message;
