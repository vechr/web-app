...
interface ITopicEventData {
  ...
  topicEventEdit: {name: string, description: string, eventExpression: object},
  error: IError
}
    ...
    topicEventList(state) {
      return (
        state.data.map((topicEvent) => ({
          ...
          createdAt: topicEvent.createdAt !== undefined ? new Date(topicEvent.createdAt).toLocaleString('en-US') : topicEvent.createdAt,
          updatedAt: topicEvent.updatedAt !== undefined ? new Date(topicEvent.updatedAt).toLocaleString('en-US') : topicEvent.updatedAt,
        }))
      );
    }
  },
    ...
    async getTopicEventById<T>(topicId: T, id: string) {
      ...
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          this.topicEventEdit.eventExpression = res.data.result.eventExpression;
